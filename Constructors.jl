...
basefiber = [exp(theta*im) for theta in range(0,(2*pi)/res,2*pi)]
...
function ConstructConnection(contacts::Vector{(Float64,Float64)},k::Int64) :: SphereConection
  l = Length(contacts)
  down = [0.0,0.0,-1.0]
  s=tan(acos(4/5)/2)
  O_prev = [1 0 0; 0 1 0; 0 0 1]
  Dev_prev = [0,0,0]
  C = [contacts[mod(n,l)] for n in 1:k]
  Theta = [acos(C[n][3]) for n in 1:k]
  Phi = [atan2(C[n][2],C[n][1]) for n in 1:k]
  Alpha = [cos(Theta[n]/2)+0im for n in 1:k]
  Beta = [exp(Phi[n]*im)*sin(Theta[n]/2)]
  O = []
  D = []
  T = []
  S = []
  ...
  for n in 1:k
    ...
    rotaxis = cross(p1,p2)
    ang = acos(dot(p1,p2))
    O_inc = SO3_from_axis_angle(axis,ang)
    O_n = O_prev * O_inc
    normal = [O_n[3,1],O_n[3,2],O_n[3,3]]
    ...
    T_vec = [0,0,1] * O_new
    T_n = [D_new[1],D_new[2],T_vec[3]]
    D_n = D_new[1]+D_new[2]im
    push!(O,O_n)
    push!(D,D_n)
    push!(T,T_n)
  ...
  return SphereConection(C,Theta,Phi,Alpha,Beta,O,D,T,S,W,A)
end


function HolomorphicTransform(conn::SphereConection,n::Int64)
  D_n = conn.D[n]
  m=abs2(D_n)
  dnum=sqrt(1+m)
  a=1/dnum
...
function Holomorph3(conn::SphereConection,n::Int64)
 H=[]
 ConeCirc=[]
 for n in 1:k
   m = abs2(D[n])
   ...
   d=a
   Circ_n=ConeCircle(acos(conn.S[3]),atan2(conn.S[2],conn.S[1]))
   H_n=[InvStereoProj(v) for v in HolomorphicTransform(conn.D[n],conn.W[n])]
   push!(H,H_n)
...
function HopfLink(conn::SphereConection,n::Int64)
  a = p
  b = a * R[n]
  theta_a = acos(a[3])
  phi_a = atan2(a[2],a[1])
  ...
  z1= basefiber * [alpha_a beta_a;-conjugate(beta_a) conjugate(alpha_a)]
  z2= basefiber * [alpha_b beta_b;-conjugate(beta_b) conjugate(alpha_b)]
  return (z1,z2)
end


function Holomorph4(conn::SphereConection,n::Int64)
  k = Length(conn.D)
  F = []
  for ind in 1:(k-3)
