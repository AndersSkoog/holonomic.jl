...
# Bezier Curve
# =============================================================
function bezier_pt(pts,t)
  n=lastindex(pts)
  x,y,z=0.0,0.0,0.0
  for k in 1:lastindex(pts)
    bern = binomial(n,k) * t^2 * ((1-t)^(n-k))
    x+=bern*pts[k][1]
    y+=bern*pts[k][2]
    z+=bern*pts[k][3]
  end
...
function bezier_curve(pts,res)
  ts=[(1/res)*ind for ind in 1:res]
...
const ANGLE_RES = 360
const angles = range(0,2π,length=ANGLE_RES)
const basefiber = ComplexF64[exp(im * θ) for θ in angles]
...
function S2_from_angles(θ::Float64, φ::Float64)::S2
    return S2(cos(φ) * cos(θ),cos(φ) * sin(θ),sin(φ))
end
...
function SO3_from_axis_angle(axis::S2,ang::Float64)::SO3
    x, y, z = axis
    c = cos(ang)
    s = sin(ang)
    C = 1 - c
...
function ConeCircle(θ::Float64,φ::Float64,res::Int = 360)
    r = sin(π / 5)
    u, v = SpherePlane(θ, φ)
    center = (4 / 5) * S2_from_angles(θ, φ)
    return S2[center + cos(t) * r * u + sin(t) * r * v for t in range(0,2π,length = res + 1)]
end
...
function InvStereoProj(v::ComplexF64)::S2
    m = abs2(v)
    return S2((2 * v.re) / (1 + m),(2 * v.im) / (1 + m),(1 - m) / (1 + m))
end
...
function ConstructConnection(contacts::Vector{Tuple{Float64, Float64}},k::Int)::SphereConnection
    l = length(contacts)
    down = S2(
        0.0,
        0.0,
    ...
    # Input angles
    # --------------------------------------------------------
    θ = [contacts[i][1] for i in 1:l]
    φ = [contacts[i][2] for i in 1:l]
    # --------------------------------------------------------
    # Sphere contacts
    # --------------------------------------------------------
    C = S2[S2_from_angles(θ[i], φ[i]) for i in 1:l]
    # --------------------------------------------------------
    # Spinor coordinates
    # --------------------------------------------------------
    α = ComplexF64[cos(θ[i] / 2) + 0im for i in 1:l]

    β = ComplexF64[exp(im * φ[i]) * sin(θ[i] / 2) for i in 1:l]
    ...
    for n in 1:k
        ...
        # Normal
        # ----------------------------------------------------
        normal = S2(O_new[3, 1],O_new[3, 2],O_new[3, 3])
        ...
        disp = move_norm < 1e-12 ? R3(0.0, 0.0, 0.0) : (ang / move_norm) * move_dir
        ...
        push!(D, D_n)
        push!(T, T_n)
        #push!(S, InvStereoProj(UnitDiscCoordinate(D_n)))
        #push!(
        #    W,
        #    UnitDiscCoordinate(s*D_n)
        #) # rotate (s*D_n) to make distinct β[n]*(s * D_n)
...
function Holomorph3(conn::SphereConnection,n::Int)
    # --------------------------------------------------------
    # Holomorphic transform
    # --------------------------------------------------------
    H = HolomorphicTransform(conn,n)

    # --------------------------------------------------------
    # Lift transformed complex curve to sphere
    # --------------------------------------------------------
    H3 = S2[InvStereoProj(z) for z in H]

    # --------------------------------------------------------
    # Cone circle corresponding to S[n]
    # --------------------------------------------------------
    s = abs(conn.D[n]) <= 1 ? InvStereoProj(conn.D[n]) : InvStereoProj(1/conj(conn.D[n]))

    θ = acos(clamp(s[3], -1.0, 1.0))
    φ = atan(s[2],s[1])
    circle = ConeCircle(θ,φ)
    return H3, circle
end
